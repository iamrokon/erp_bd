...
if ($new)
	customer_list_row(_("From Customer:"), 'customer_id', null, false, true, false, false, $dimensionInfo['dim1'], $dimensionInfo['dim2']);
else {
...
if (db_customer_has_branches($_POST['customer_id'])) {
	customer_branches_list_row(_("Company|Branch:"), $_POST['customer_id'], 'BranchID', null, false, true, true);
} else {
...
function get_bank_acc_info()
{
	global $_SESSION;
	$user_id = $_SESSION['wa_current_user']->user;
	$sql = "SELECT bank_accounts_number
					FROM `0_users`
					WHERE id='$user_id'";

	$bank_acc_n = db_query($sql, "Cant retrieve invoice range");
	$bank_acc_Info = db_fetch($bank_acc_n);

	return $bank_acc_Info['bank_accounts_number'];
}

$bank_acc_number = get_bank_acc_info();
bank_accounts_list_row(_("Into Bank Account:"), 'bank_account', null, true, false, $bank_acc_number);
