...
function get_dimensionss_info()
{
	global $_SESSION;
	$user_pos = $_SESSION['wa_current_user']->pos;
	$sql = "SELECT dim1,
					dim2, pos_location, s_area
					FROM `0_sales_pos`
					WHERE id='$user_pos'";
...
{
	global $SysPrefs, $path_to_root, $page_nested, $dimensionInfo;
	...
	text_row(_("Customer Short Name:"), 'cust_ref', null, 50, 30);
	text_row(_("Address:"), 'address', $_POST['address'], 35, 200);

	text_row(_("VAT | GSTNo:"), 'tax_id', null, 40, 40);
	...
	elseif (isset($SysPrefs->auto_create_branch) && $SysPrefs->auto_create_branch == 1)
	{
		...
		text_row(_("Website:"), 'fax', null, 32, 30);
		text_row(_("Bank Account Number:"), 'bank_account', null, 30, 60);
		sales_persons_list_row( _("Sales Person:"), 'salesman', null);
		sales_areas_list_row( _("Sales Area:"), 'area', null, $dimensionInfo['s_area']);
	}
	table_section(2);
	...
	if ($dim >= 1)
		dimensions_list_row(_("Select Company")." :", 'dimension_id', $_POST['dimension_id'], true, " ", false, 1, false, $dimensionInfo['dim1']);
	if ($dim > 1)
		dimensions_list_row(_("Select Branch")." :", 'dimension2_id', $_POST['dimension2_id'], true, " ", false, 2);
	if ($dim < 1)
	...
	text_row(_("General Notes:"), 'notes', null, 35, 200);
	if (!$selected_id && isset($SysPrefs->auto_create_branch) && $SysPrefs->auto_create_branch == 1)
	{


		locations_list_row(_("Default Inventory Location:"), 'location', null, false, false, false, false, $dimensionInfo['pos_location']);
...
if (db_has_customers())
{
	...
	customer_list_cells(_("Select a customer: "), 'customer_id', null,
		_('New customer'), true, check_value('show_inactive'), false, false, $dimensionInfo['dim1'], $dimensionInfo['dim2']);
